...
export default function EngineeringNotes() {
  return (
    <section className="border-t border-eternal-border py-24">
      <div className="inner">
        <FadeUp>
        ...
        <div className="grid grid-cols-1 gap-3 min-[560px]:grid-cols-2 min-[900px]:grid-cols-3">
          {products.map((product, index) => (
            <FadeUp key={product.id} delay={Math.min(index, 2) * 0.05}>
              <div className="flex h-full flex-col gap-2 rounded-[10px] border border-eternal-border bg-white/[0.015] p-[18px]">
                <div className="text-[13px] font-semibold tracking-[-0.01em] text-eternal-text">
                  {product.name}
                </div>
                <p className="text-pretty text-[13px] leading-[1.55] tracking-[-0.003em] text-eternal-text-secondary">
                  {product.description}
                </p>
              </div>
            </FadeUp>
